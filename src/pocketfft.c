...
[[nodiscard]] static void *checked_calloc(size_t count, size_t size) {
  if ((count == 0) || (size == 0)) return nullptr;
  if (count > (SIZE_MAX / size)) return nullptr;
  return calloc(count, size);
}

[[nodiscard]] static bool size_add(size_t a, size_t b, size_t *out) {
  if (a > (SIZE_MAX - b)) return false;
  *out = a + b;
  return true;
}

[[nodiscard]] static bool size_mul(size_t a, size_t b, size_t *out) {
  if ((a == 0) || (b == 0)) {
    *out = 0;
    return true;
  }
...
[[nodiscard]] static bool size_sub(size_t a, size_t b, size_t *out) {
  if (a < b) return false;
  *out = a - b;
  return true;
}
...
static NOINLINE void calc_first_octant(size_t den, double *restrict res) {
  size_t n = (den + 4) >> 3;
  if (n == 0) return;
  res[0] = 1.;
  res[1] = 0.;
  ...
  for (size_t i = 1; i < l1; ++i) my_sincosm1pi((2. * i) / den, &res[2 * i]);
  size_t start = l1;
  while (start < n) {
    double cs[2];
    ...
    for (size_t i = 1; i < end; ++i) {
      ...
      res[2 * (start + i)] =
          ((cs[0] * csx[0] - cs[1] * csx[1] + cs[0]) + csx[0]) + 1.;
      res[2 * (start + i) + 1] =
          (cs[0] * csx[1] + cs[1] * csx[0]) + cs[1] + csx[1];
    }
    start += l1;
  }
  for (size_t i = 1; i < l1; ++i) res[2 * i] += 1.;
}

static NOINLINE void calc_first_quadrant(size_t n, double *restrict res) {
...
static NOINLINE void calc_first_half(size_t n, double *restrict res) {
  int ndone = (n + 1) >> 1;
  double *p = res + n - 1;
...
static NOINLINE void fill_first_quadrant(size_t n, double *restrict res) {
  constexpr double hsqt2 = 0.707106781186547524400844362104849;
  size_t quart = n >> 2;
  if ((n & 7) == 0) res[quart] = res[quart + 1] = hsqt2;
...
static NOINLINE void fill_first_half(size_t n, double *restrict res) {
  size_t half = n >> 1;
  if ((n & 3) == 0)
...
static NOINLINE void fill_second_half(size_t n, double *restrict res) {
  if ((n & 1) == 0)
    for (size_t i = 0; i < n; ++i) res[i + n] = -res[i];
  else
...
static NOINLINE void sincos_2pibyn_half(size_t n, double *restrict res) {
  if ((n & 3) == 0) {
...
static NOINLINE void sincos_2pibyn(size_t n, double *restrict res) {
...
[[nodiscard]] static NOINLINE bool good_size(size_t n, size_t *out) {
  if (n <= 6) {
    *out = n;
    return true;
  }
  ...
  size_t bestfac;
  if (!size_mul(2, n, &bestfac)) return false;

  for (size_t f2 = 1; f2 < bestfac;) {
    for (size_t f23 = f2; f23 < bestfac;) {
      for (size_t f235 = f23; f235 < bestfac;) {
        for (size_t f2357 = f235; f2357 < bestfac;) {
          for (size_t f235711 = f2357; f235711 < bestfac;) {
            if (f235711 >= n) bestfac = f235711;
            size_t next;
            if (!size_mul(f235711, 11, &next)) break;
            f235711 = next;
          }
          size_t next;
          if (!size_mul(f2357, 7, &next)) break;
          f2357 = next;
        }
        size_t next;
        if (!size_mul(f235, 5, &next)) break;
        f235 = next;
      }
      size_t next;
      if (!size_mul(f23, 3, &next)) break;
      f23 = next;
    }
    size_t next;
    if (!size_mul(f2, 2, &next)) break;
    f2 = next;
  }
  *out = bestfac;
  return true;
...
static constexpr size_t NFCT = 25;
...
/* a = b*c */
#define A_EQ_B_MUL_C(a, b, c)    \
  {                              \
    a.r = b.r * c.r - b.i * c.i; \
    a.i = b.r * c.i + b.i * c.r; \
  }
/* a = conj(b)*c*/
#define A_EQ_CB_MUL_C(a, b, c)   \
  {                              \
    a.r = b.r * c.r + b.i * c.i; \
    a.i = b.r * c.i - b.i * c.r; \
  }
...
static NOINLINE void pass2b(size_t ido, size_t l1, const cmplx *restrict cc,
...
static NOINLINE void pass2f(size_t ido, size_t l1, const cmplx *restrict cc,
                            cmplx *restrict ch, const cmplx *restrict wa) {
  const size_t cdim = 2;
...
static NOINLINE void pass3b(size_t ido, size_t l1, const cmplx *restrict cc,
                            cmplx *restrict ch, const cmplx *restrict wa) {
  const size_t cdim = 3;
  constexpr double tw1r = -0.5;
  constexpr double tw1i = 0.86602540378443864676;
...
static NOINLINE void pass3f(size_t ido, size_t l1, const cmplx *restrict cc,
                            cmplx *restrict ch, const cmplx *restrict wa) {
  const size_t cdim = 3;
  constexpr double tw1r = -0.5;
  constexpr double tw1i = -0.86602540378443864676;
...
static NOINLINE void pass4b(size_t ido, size_t l1, const cmplx *restrict cc,
                            cmplx *restrict ch, const cmplx *restrict wa) {
  const size_t cdim = 4;
  ...
  if (ido == 1)
    ...
    for (size_t k = 0; k < l1; ++k) {
      ...
      for (size_t i = 1; i < ido; ++i) {
        ...
        cmplx cc0 = CC(i, 0, k), cc1 = CC(i, 1, k), cc2 = CC(i, 2, k),
              cc3 = CC(i, 3, k);
        PMC(t2, t1, cc0, cc2)
        PMC(t3, t4, cc1, cc3)
        ROT90(t4)
...
static NOINLINE void pass4f(size_t ido, size_t l1, const cmplx *restrict cc,
                            cmplx *restrict ch, const cmplx *restrict wa) {
  const size_t cdim = 4;
...
static NOINLINE void pass5b(size_t ido, size_t l1, const cmplx *restrict cc,
                            cmplx *restrict ch, const cmplx *restrict wa) {
  const size_t cdim = 5;
  constexpr double tw1r = 0.3090169943749474241;
  constexpr double tw1i = 0.95105651629515357212;
  constexpr double tw2r = -0.8090169943749474241;
  constexpr double tw2i = 0.58778525229247312917;
...
static NOINLINE void pass5f(size_t ido, size_t l1, const cmplx *restrict cc,
                            cmplx *restrict ch, const cmplx *restrict wa) {
  const size_t cdim = 5;
  constexpr double tw1r = 0.3090169943749474241;
  constexpr double tw1i = -0.95105651629515357212;
...
static NOINLINE void pass7(size_t ido, size_t l1, const cmplx *restrict cc,
                           cmplx *restrict ch, const cmplx *restrict wa,
                           const int sign) {
  const size_t cdim = 7;
  constexpr double tw1r = 0.623489801858733530525;
  const double tw1i = sign * 0.7818314824680298087084;
  constexpr double tw2r = -0.222520933956314404289;
  const double tw2i = sign * 0.9749279121818236070181;
  constexpr double tw3r = -0.9009688679024191262361;
  const double tw3i = sign * 0.4338837391175581204758;
...
#define PARTSTEP11a0(u1, u2, x1, x2, x3, x4, x5, y1, y2, y3, y4, y5, out1,   \
                     out2)                                                   \
  {                                                                          \
    cmplx ca, cb;                                                            \
...
#define PARTSTEP11a(u1, u2, x1, x2, x3, x4, x5, y1, y2, y3, y4, y5)          \
  PARTSTEP11a0(u1, u2, x1, x2, x3, x4, x5, y1, y2, y3, y4, y5, CH(0, k, u1), \
               CH(0, k, u2))
...
static NOINLINE void pass11(size_t ido, size_t l1, const cmplx *restrict cc,
                            cmplx *restrict ch, const cmplx *restrict wa,
                            const int sign) {
  const size_t cdim = 11;
  constexpr double tw1r = 0.8412535328311811688618;
  const double tw1i = sign * 0.5406408174555975821076;
  constexpr double tw2r = 0.4154150130018864255293;
  const double tw2i = sign * 0.9096319953545183714117;
  constexpr double tw3r = -0.1423148382732851404438;
  const double tw3i = sign * 0.9898214418809327323761;
  constexpr double tw4r = -0.6548607339452850640569;
  const double tw4i = sign * 0.755749574354258283774;
  constexpr double tw5r = -0.9594929736144973898904;
  const double tw5i = sign * 0.2817325568414296977114;

  if (ido == 1)
    for (size_t k = 0; k < l1; ++k) {
      PREP11(0)
      PARTSTEP11a(1, 10, tw1r, tw2r, tw3r, tw4r, tw5r, +tw1i, +tw2i, +tw3i,
                  +tw4i, +tw5i) PARTSTEP11a(2, 9, tw2r, tw4r, tw5r, tw3r, tw1r,
                                            +tw2i, +tw4i, -tw5i, -tw3i, -tw1i)
          PARTSTEP11a(3, 8, tw3r, tw5r, tw2r, tw1r, tw4r, +tw3i, -tw5i, -tw2i,
                      +tw1i, +tw4i)
              PARTSTEP11a(4, 7, tw4r, tw3r, tw1r, tw5r, tw2r, +tw4i, -tw3i,
                          +tw1i, +tw5i, -tw2i)
                  PARTSTEP11a(5, 6, tw5r, tw1r, tw4r, tw2r, tw3r, +tw5i, -tw1i,
                              +tw4i, -tw2i, +tw3i)
    }
  else
    for (size_t k = 0; k < l1; ++k) {
      {
        ...
        PARTSTEP11a(1, 10, tw1r, tw2r, tw3r, tw4r, tw5r, +tw1i, +tw2i, +tw3i,
                    +tw4i, +tw5i)
            PARTSTEP11a(2, 9, tw2r, tw4r, tw5r, tw3r, tw1r, +tw2i, +tw4i, -tw5i,
                        -tw3i, -tw1i)
                PARTSTEP11a(3, 8, tw3r, tw5r, tw2r, tw1r, tw4r, +tw3i, -tw5i,
                            -tw2i, +tw1i, +tw4i)
                    PARTSTEP11a(4, 7, tw4r, tw3r, tw1r, tw5r, tw2r, +tw4i,
                                -tw3i, +tw1i, +tw5i, -tw2i)
                        PARTSTEP11a(5, 6, tw5r, tw1r, tw4r, tw2r, tw3r, +tw5i,
                                    -tw1i, +tw4i, -tw2i, +tw3i)
      }
      for (size_t i = 1; i < ido; ++i) {
        PREP11(i)
        PARTSTEP11(1, 10, tw1r, tw2r, tw3r, tw4r, tw5r, +tw1i, +tw2i, +tw3i,
                   +tw4i, +tw5i)
        PARTSTEP11(2, 9, tw2r, tw4r, tw5r, tw3r, tw1r, +tw2i, +tw4i, -tw5i,
                   -tw3i, -tw1i)
        PARTSTEP11(3, 8, tw3r, tw5r, tw2r, tw1r, tw4r, +tw3i, -tw5i, -tw2i,
                   +tw1i, +tw4i)
        PARTSTEP11(4, 7, tw4r, tw3r, tw1r, tw5r, tw2r, +tw4i, -tw3i, +tw1i,
                   +tw5i, -tw2i)
        PARTSTEP11(5, 6, tw5r, tw1r, tw4r, tw2r, tw3r, +tw5i, -tw1i, +tw4i,
                   -tw2i, +tw3i)
      }
    }
}
...
static NOINLINE int passg(size_t ido, size_t ip, size_t l1, cmplx *restrict cc,
                          cmplx *restrict ch, const cmplx *restrict wa,
  ...
  for (size_t k = 0; k < l1; ++k)
    for (size_t i = 0; i < ido; ++i) {
      cmplx tmp = CH(i, k, 0);
      for (size_t j = 1; j < ipph; ++j) ADDC(tmp, tmp, CH(i, k, j))
      CX(i, k, 0) = tmp;
    }
  for (size_t l = 1, lc = ip - 1; l < ipph; ++l, --lc) {
    ...
    for (size_t ik = 0; ik < idl1; ++ik) {
      CX2(ik, l).r =
          CH2(ik, 0).r + wal[l].r * CH2(ik, 1).r + wal[2 * l].r * CH2(ik, 2).r;
      CX2(ik, l).i =
          CH2(ik, 0).i + wal[l].r * CH2(ik, 1).i + wal[2 * l].r * CH2(ik, 2).i;
      CX2(ik, lc).r =
          -wal[l].i * CH2(ik, ip - 1).i - wal[2 * l].i * CH2(ik, ip - 2).i;
      CX2(ik, lc).i =
...
WARN_UNUSED_RESULT static int pass_all(cfftp_plan plan, cmplx c[], double fct,
  ...
  for (size_t k1 = 0; k1 < nf; k1++) {
    ...
    if (ip == 4)
      ...
      pass7(ido, l1, p1, p2, plan->fct[k1].tw, sign);
    else if (ip == 11)
      pass11(ido, l1, p1, p2, plan->fct[k1].tw, sign);
    else {
      if (passg(ido, ip, l1, p1, p2, plan->fct[k1].tw, plan->fct[k1].tws,
                sign)) {
        DEALLOC(ch);
        return -1;
      }
  ...
  if (p1 != c) {
    if (fct != 1.)
      ...
      size_t copy_bytes;
      if (!size_mul(len, sizeof(cmplx), &copy_bytes)) {
        DEALLOC(ch);
        return -1;
      }
      memcpy(c, p1, copy_bytes);
    }
  } else if (fct != 1.)
    for (size_t i = 0; i < len; ++i) {
      c[i].r *= fct;
...
WARN_UNUSED_RESULT static int cfftp_forward(cfftp_plan plan, double c[],
                                            double fct) {
  return pass_all(plan, (cmplx *)c, fct, -1);
}

WARN_UNUSED_RESULT static int cfftp_backward(cfftp_plan plan, double c[],
                                             double fct) {
  return pass_all(plan, (cmplx *)c, fct, 1);
}

WARN_UNUSED_RESULT static int cfftp_factorize(cfftp_plan plan) {
...
[[nodiscard]] static NOINLINE bool cfftp_twsize(cfftp_plan plan,
                                                size_t *twsize_out) {
  size_t twsize = 0, l1 = 1;
  for (size_t k = 0; k < plan->nfct; ++k) {
    size_t l1_ip;
    if (!size_mul(l1, plan->fct[k].fct, &l1_ip)) return false;
    size_t ip = plan->fct[k].fct, ido = plan->length / l1_ip;
    size_t twiddle_terms;
    if (!size_mul(ip - 1, ido - 1, &twiddle_terms)) return false;
    if (!size_add(twsize, twiddle_terms, &twsize)) return false;
    if ((ip > 11) && !size_add(twsize, ip, &twsize)) return false;
    l1 = l1_ip;
  }
  *twsize_out = twsize;
  return true;
...
WARN_UNUSED_RESULT static int cfftp_comp_twiddle(cfftp_plan plan) {
  size_t length = plan->length;
  size_t twid_count;
  if (!size_mul(2, length, &twid_count)) return -1;
  double *twid = RALLOC(double, twid_count);
  if (!twid) return -1;
...
[[nodiscard]] static cfftp_plan make_cfftp_plan(size_t length) {
  ...
  for (size_t i = 0; i < NFCT; ++i)
    plan->fct[i] = (cfftp_fctdata){.fct = 0, .tw = nullptr, .tws = nullptr};
  plan->mem = nullptr;
  ...
  size_t tws;
  if (!cfftp_twsize(plan, &tws)) {
    DEALLOC(plan);
    return nullptr;
  }
  if (tws == 0) return plan;
...
static NOINLINE void radf2(size_t ido, size_t l1, const double *restrict cc,
...
static NOINLINE void radf3(size_t ido, size_t l1, const double *restrict cc,
                           double *restrict ch, const double *restrict wa) {
  const size_t cdim = 3;
  static constexpr double taur = -0.5;
  static constexpr double taui = 0.86602540378443864676;
...
static NOINLINE void radf4(size_t ido, size_t l1, const double *restrict cc,
                           double *restrict ch, const double *restrict wa) {
  const size_t cdim = 4;
  static constexpr double hsqt2 = 0.70710678118654752440;
  ...
  for (size_t k = 0; k < l1; k++)
    for (size_t i = 2; i < ido; i += 2) {
      size_t ic = ido - i;
      double ci2, ci3, ci4, cr2, cr3, cr4, ti1, ti2, ti3, ti4, tr1, tr2, tr3,
          tr4;
...
static NOINLINE void radf5(size_t ido, size_t l1, const double *restrict cc,
                           double *restrict ch, const double *restrict wa) {
  const size_t cdim = 5;
  static constexpr double tr11 = 0.3090169943749474241;
  static constexpr double ti11 = 0.95105651629515357212;
  static constexpr double tr12 = -0.8090169943749474241;
  static constexpr double ti12 = 0.58778525229247312917;
  ...
  for (size_t k = 0; k < l1; ++k)
    for (size_t i = 2; i < ido; i += 2) {
      double ci2, di2, ci4, ci5, di3, di4, di5, ci3, cr2, cr3, dr2, dr3, dr4,
...
#define CC(a, b, c) cc[(a) + ido * ((b) + cdim * (c))]
#define CH(a, b, c) ch[(a) + ido * ((b) + l1 * (c))]
static NOINLINE void radfg(size_t ido, size_t ip, size_t l1,
                           double *restrict cc, double *restrict ch,
                           const double *restrict wa,
  ...
  if (ido > 1) {
    for (size_t j = 1, jc = ip - 1; j < ipph; ++j, --jc)  // 114
    {
      ...
      for (size_t k = 0; k < l1; ++k)  // 113
      {
        ...
        {
          double t1 = C1(i, k, j), t2 = C1(i + 1, k, j), t3 = C1(i, k, jc),
                 t4 = C1(i + 1, k, jc);
  ...
  for (size_t l = 1, lc = ip - 1; l < ipph; ++l, --lc)  // 127
  {
    for (size_t ik = 0; ik < idl1; ++ik)  // 124
    {
      ...
      CH2(ik, lc) =
          csarr[2 * l + 1] * C2(ik, ip - 1) + csarr[4 * l + 1] * C2(ik, ip - 2);
    }
    size_t iang = 2 * l;
    size_t j = 3, jc = ip - 3;
...
static NOINLINE void radb2(size_t ido, size_t l1, const double *restrict cc,
                           double *restrict ch, const double *restrict wa) {
  const size_t cdim = 2;
...
static NOINLINE void radb3(size_t ido, size_t l1, const double *restrict cc,
                           double *restrict ch, const double *restrict wa) {
  const size_t cdim = 3;
  static constexpr double taur = -0.5;
  static constexpr double taui = 0.86602540378443864676;
...
static NOINLINE void radb4(size_t ido, size_t l1, const double *restrict cc,
                           double *restrict ch, const double *restrict wa) {
  const size_t cdim = 4;
...
static NOINLINE void radb5(size_t ido, size_t l1, const double *restrict cc,
                           double *restrict ch, const double *restrict wa) {
  const size_t cdim = 5;
  static constexpr double tr11 = 0.3090169943749474241;
  static constexpr double ti11 = 0.95105651629515357212;
...
static NOINLINE void radbg(size_t ido, size_t ip, size_t l1,
                           double *restrict cc, double *restrict ch,
                           const double *restrict wa,
                           const double *restrict csarr) {
  const size_t cdim = ip;
  ...
  for (size_t l = 1, lc = ip - 1; l < ipph; ++l, --lc) {
    for (size_t ik = 0; ik < idl1; ++ik) {
      C2(ik, l) =
          CH2(ik, 0) + csarr[2 * l] * CH2(ik, 1) + csarr[4 * l] * CH2(ik, 2);
...
[[nodiscard]] static int copy_and_norm(double *c, double *p1, size_t n,
                                       double fct) {
  if (p1 != c) {
    if (fct != 1.)
      for (size_t i = 0; i < n; ++i) c[i] = fct * p1[i];
    else {
      size_t copy_bytes;
      if (!size_mul(n, sizeof(double), &copy_bytes)) return -1;
      memcpy(c, p1, copy_bytes);
    }
  } else if (fct != 1.)
    for (size_t i = 0; i < n; ++i) c[i] *= fct;
  return 0;
}
...
static int rfftp_backward(rfftp_plan plan, double c[], double fct) {
  ...
  for (size_t k = 0; k < nf; k++) {
    size_t ip = plan->fct[k].fct, ido = n / (ip * l1);
    if (ip == 4)
...
[[nodiscard]] static bool rfftp_twsize(rfftp_plan plan, size_t *twsize_out) {
  size_t twsize = 0, l1 = 1;
  for (size_t k = 0; k < plan->nfct; ++k) {
    size_t l1_ip;
    if (!size_mul(l1, plan->fct[k].fct, &l1_ip)) return false;
    ...
    if (ip > 5) {
      size_t extra_terms;
      if (!size_mul(2, ip, &extra_terms)) return false;
      if (!size_add(twsize, extra_terms, &twsize)) return false;
    }
    l1 = l1_ip;
  }
  *twsize_out = twsize;
    ...
    if (k < plan->nfct - 1)  // last factor doesn't need twiddles
    {
      ...
      for (size_t j = 1; j < ip; ++j)
        for (size_t i = 1; i <= (ido - 1) / 2; ++i) {
          plan->fct[k].tw[(j - 1) * (ido - 1) + 2 * i - 2] =
              twid[2 * j * l1 * i];
          plan->fct[k].tw[(j - 1) * (ido - 1) + 2 * i - 1] =
              twid[2 * j * l1 * i + 1];
        }
    }
...
[[nodiscard]] static NOINLINE rfftp_plan make_rfftp_plan(size_t length) {
  if (length == 0) return nullptr;
  ...
  plan->mem = nullptr;
  for (size_t i = 0; i < NFCT; ++i)
    plan->fct[i] = (rfftp_fctdata){.fct = 0, .tw = nullptr, .tws = nullptr};
  if (length == 1) return plan;
...
static NOINLINE void destroy_rfftp_plan(rfftp_plan plan) {
  if (plan == nullptr) return;
  DEALLOC(plan->mem);
  DEALLOC(plan);
}
...
[[nodiscard]] static bool fftblue_length_to_n2(size_t length, size_t *n2_out) {
  size_t twice_length;
  if (!size_mul(2, length, &twice_length)) return false;
  size_t bluestein_length;
  if (!size_sub(twice_length, 1, &bluestein_length)) return false;
  return good_size(bluestein_length, n2_out);
}

[[nodiscard]] static NOINLINE fftblue_plan make_fftblue_plan(size_t length) {
  fftblue_plan plan = RALLOC(fftblue_plan_i, 1);
  if (!plan) return nullptr;
  plan->plan = nullptr;
  plan->n = length;
  if (!fftblue_length_to_n2(plan->n, &plan->n2)) {
    DEALLOC(plan);
    return nullptr;
  }
  size_t twice_n;
  size_t twice_n2;
  size_t mem_count;
  if (!size_mul(2, plan->n, &twice_n) || !size_mul(2, plan->n2, &twice_n2) ||
      !size_add(twice_n, twice_n2, &mem_count)) {
    DEALLOC(plan);
    return nullptr;
  }
  plan->mem = RALLOC(double, mem_count);
  if (!plan->mem) {
    DEALLOC(plan);
    return nullptr;
  }
  plan->bk = plan->mem;
  plan->bkf = plan->bk + twice_n;

  /* initialize b_k */
  size_t tmp_count;
  if (!size_mul(4, plan->n, &tmp_count)) {
    DEALLOC(plan->mem);
    DEALLOC(plan);
    return nullptr;
  }
  double *tmp = RALLOC(double, tmp_count);
  if (!tmp) {
    DEALLOC(plan->mem);
    DEALLOC(plan);
    return nullptr;
  }
  sincos_2pibyn(twice_n, tmp);
  plan->bk[0] = 1;
  plan->bk[1] = 0;
  ...
  for (size_t m = 1; m < plan->n; ++m) {
    coeff += 2 * m - 1;
    if (coeff >= twice_n) coeff -= twice_n;
  ...
  for (size_t m = 2; m < twice_n; m += 2) {
    plan->bkf[m] = plan->bkf[twice_n2 - m] = plan->bk[m] * xn2;
    plan->bkf[m + 1] = plan->bkf[twice_n2 - m + 1] = plan->bk[m + 1] * xn2;
  }
  for (size_t m = twice_n; m <= (twice_n2 - twice_n + 1); ++m)
    plan->bkf[m] = 0.;
...
WARN_UNUSED_RESULT static int fftblue_fft(fftblue_plan plan, double c[],
                                          int isign, double fct) {
  size_t n = plan->n;
  size_t n2 = plan->n2;
  double *bk = plan->bk;
  double *bkf = plan->bkf;
  size_t akf_count;
  if (!size_mul(2, n2, &akf_count)) return -1;
  double *akf = RALLOC(double, akf_count);
  if (!akf) return -1;
  ...
  for (size_t m = 2 * n; m < 2 * n2; ++m) akf[m] = 0;
...
static int rfftblue_backward(fftblue_plan plan, double c[], double fct) {
  ...
  size_t tmp_count;
  size_t copy_bytes;
  if (!size_mul(2, n, &tmp_count) ||
      !size_mul(n - 1, sizeof(double), &copy_bytes))
    return -1;
  double *tmp = RALLOC(double, tmp_count);
  if (!tmp) return -1;
  tmp[0] = c[0];
  tmp[1] = 0.;
  memcpy(tmp + 2, c + 1, copy_bytes);
  if ((n & 1) == 0) tmp[n + 1] = 0.;
  ...
  for (size_t m = 0; m < n; ++m) c[m] = tmp[2 * m];
  DEALLOC(tmp);
  return 0;
}
...
[[nodiscard]] cfft_plan make_cfft_plan(size_t length) {
  ...
  size_t n2;
  if (!fftblue_length_to_n2(length, &n2)) {
    plan->packplan = make_cfftp_plan(length);
    if (!plan->packplan) {
      DEALLOC(plan);
      return nullptr;
...
void destroy_cfft_plan(cfft_plan plan) {
  if (plan == nullptr) return;
  if (plan->blueplan) destroy_fftblue_plan(plan->blueplan);
  if (plan->packplan) destroy_cfftp_plan(plan->packplan);
  DEALLOC(plan);
}
...
void destroy_rfft_plan(rfft_plan plan) {
  if (plan == nullptr) return;
  if (plan->blueplan) destroy_fftblue_plan(plan->blueplan);
  if (plan->packplan) destroy_rfftp_plan(plan->packplan);
  DEALLOC(plan);
}
...
[[nodiscard]] size_t rfft_length(rfft_plan plan) {
  ...
  if (plan->blueplan) return plan->blueplan->n;
  return 0;
}
